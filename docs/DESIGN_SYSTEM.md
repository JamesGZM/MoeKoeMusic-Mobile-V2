...
- 手机普通确认 Dialog 默认宽度为 `min(264.dp, windowWidth - 64.dp)`；六位验证码等结构化输入 Dialog 可使用 `min(288.dp, windowWidth - 64.dp)`。两者由 390dp 登录画布校准，高度均由内容决定，不通过放大字号和按钮填满容器，也不机械沿用 `18-mobile-states-overlays.png` 中偏大的示例宽度。Dialog 默认相对完整可用视口水平、垂直居中，不得相对底部表单或剩余内容区居中。遮罩约为 32% 黑色，确认型操作沿用 [`11-dialog-components.png`](design/mockups/11-dialog-components.png) 的左侧文字取消与右侧主按钮结构。
