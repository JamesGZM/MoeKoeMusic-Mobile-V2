...
| `titleMedium` | `18sp / 26sp` | SemiBold | 标准 Toolbar、卡片和 Dialog 标题 |
...
1. 标准：以 `10-user-profile.png` 为基准。高度 `64dp`，不含系统状态栏；水平内容边距 `16dp`；标题使用单行 `titleMedium`（`18sp / 26sp`），相对完整页面宽度几何居中，而不是跟随左侧内容起排。共享 Chevron 使用自身已确认路径尺寸；尾部 Material 操作使用 `20dp` 可见图形，均置于至少 `48dp` 语义触控区，不显示圆形底板、卡片外框或 Toolbar 整体阴影。
