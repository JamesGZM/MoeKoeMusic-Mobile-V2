...
- 带语义前导图标的输入框以已确认登录输入框为母版：图标不是裸放，也不使用圆形底板，而是居中放入低强调的圆角方形 `primaryContainer`。Compact 输入使用约 `30dp` 容器、`8dp` 圆角和 `18dp` 图标；Default 输入使用约 `32dp` 容器、`8dp` 圆角和 `20dp` 图标。浅色主题容器使用 `primaryContainer` 约 56% 强度，图标使用 `primary`，并可保留极弱的 Primary 描边。
