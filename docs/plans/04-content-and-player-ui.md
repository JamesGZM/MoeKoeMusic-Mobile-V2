...
- 全屏歌词页纯 UI 已按 [`PLAYER_LYRICS_LAYOUT_SPEC.md`](../design/PLAYER_LYRICS_LAYOUT_SPEC.md)、`07-player-lyrics.png` 与 `23a` 至 `23h` 完成第二轮同画布复刻：纠正旧证据未拦住的不同歌词密度整体下沉、加载态锚点、行间距，以及 Compact 收藏与进度细节；封面/歌词仍使用真实双页 Pager，歌词页复用顶部栏和底部播放骨架。加载、空、离线、错误、原文、翻译、音译及 `150%` / `200%` 均有逐状态设计并排证据；真实歌词加载、缓存、同步与点击 seek 保持为后续业务切片。
