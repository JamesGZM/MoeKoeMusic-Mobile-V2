...
状态：进行中。音乐内容组件、搜索/本地列表、MiniPlayer、全屏封面/歌词、队列重排、动态色与歌词业务接线已完成；其余内容详情仍待实现。
...
- 播放队列已按 [`PLAYER_QUEUE_LAYOUT_SPEC.md`](../design/PLAYER_QUEUE_LAYOUT_SPEC.md) 和 `08-player-queue.png` 完成第二轮同画布复刻：系统 `ModalBottomSheet` 内的几何锚点保持一致，并纠正旧证据未拦住的 Sheet 大面积容器色和当前项 Surface 偏蓝问题；标准、空队列、`1.5×` 共 3 组截图基线及主态并排/叠加/差异证据已更新。非 Shuffle 模式下，所有项（含当前项）从行尾 `22dp` 点阵的独立 `48dp` 节点长按后以 stable-id 临时顺序拖动，drop 一次解析为最新 Media3 最终索引；Rejected/stale、权威顺序变化、Sheet 关闭或 2 秒未对齐均回滚。点阵与移除绝对位置不变，但两个非重叠 `48dp` 动作区占用额外 `17dp` 尾部布局宽度，已限于三张队列 golden 复核。随机、空和单项保持逐行禁用；TalkBack 提供首末裁减的上移/下移动作。真机拖动、惯性与横纵手势仍待验。
...
- 播放器动态色、歌词逐行交互，以及歌单/专辑/歌手/排行榜详情和对应完整状态矩阵。
- 发现页后续真实榜单、歌单与播放纵向切片复用首页已经稳定的数据与组件能力。
- 队列拖拽真机、封面/歌词切换、AMOLED 内容页面与 `2.0×` 字体真机关键控制验收。
