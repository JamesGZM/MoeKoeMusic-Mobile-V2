...
- “我的”顶部账户卡片、匿名态、加载/错误状态、响应式布局和本地音乐入口已经形成可运行实现，但匿名视觉仍需按候选稿确认后校准。匿名用户点击签到、VIP、云盘、历史、收藏、关注或歌单等账号资产时统一进入登录页，不显示“登录后可用”“待接入”等重复门禁文案，也不展示假计数或假列表；本地音乐与应用级设置保持可用。
...
- “我的”已登录主态继续使用确认设计 `04-my-v4.png`；未登录状态当前候选为 [`my-anonymous-v3`](../design/mockups/candidates/my-anonymous-v3/README.md)，确认前不得作为 Compose 实现基线。未来新增会话失效等未覆盖布局时仍须先补静态设计图并确认，之后才能制作必要原型或编码。
