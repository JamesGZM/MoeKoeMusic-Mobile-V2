...
当前已完成第 1、2、3 项及第 4 项的代码/自动验收子切片：密码请求、风险挑战/方式/提交协议、Repository 领域基础、密码表单、一次重试 ViewModel、短信验证 UI、基于 AndroidX WebKit 的非导出腾讯 Activity、二维码 key/check Endpoint、`0/1/2/4` 类型化 Decoder、扫码成功后的原子会话提交、ZXing 高对比二维码、2 秒生命周期轮询、120 秒本地过期截止、离页/刷新取消、连续三次失败恢复，以及生成/等待/已扫/过期/失败五态截图。2026-08-06 又按确认稿完成密码风险、短信风险和腾讯失败覆盖层返工，普通确认弹窗与短信输入弹窗分别收敛为 `304dp` / `320dp`，并统一高层级容器色、标题/正文排版、Tonal 取消按钮和 `48dp` 操作按钮；同时补齐密码页标题与多账号未选择/已选择/失效恢复矩阵，并统一扫码五态的标题、使用说明、步骤和安全页脚。生成态移除了空二维码占位框，并改用不携带虚假百分比的不确定进度；已扫码态改用独立确认卡并保留倒计时，失败态也已收敛为网络提示、居中重试按钮和文字式验证码入口，过期态刷新操作已改为居中紧凑按钮。登录模块现有 26 张截图基准自动通过，手机号主状态覆盖浅色、深色和 AMOLED，深色 Hero 使用主题感知遮罩保持固定亮色插画上的文字对比；多账号验证码失效态另以 `2.0×` 字体在 ELE-AL00 / API 29 上自动滚动并点击恢复按钮，二维码生成态的不确定进度语义、登录方式分段控件的 Tab/选中语义，以及多账号列表的单一 RadioButton 选择语义与单次切换回调也通过同一真机自动测试。密码入口、系统返回、腾讯 Activity 非导出、扫码入口、真实 key 生成和离页切换已通过同一真机测试；设备截图经项目同版 ZXing 解码并匹配固定登录 URL 前缀，未保存到仓库或输出 key。第 4 项仅剩另一台已登录酷狗设备主动扫码后的状态 `2→4`、原子会话恢复验收；用户主动风控兼容验收也尚未完成。固定 Node 请求层生成的模拟 `sid/edt` 不迁移，原生响应缺失和腾讯实际资源域名按审计保留为真机验收项。
