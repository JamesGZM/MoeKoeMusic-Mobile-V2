...
状态：进行中（确认稿视觉门禁已完成；真实账号、风控、扫码与会话恢复仍保留人工/服务验收）。确认日期：2026-08-05；适配契约确认日期：2026-08-07；视觉复验完成日期：2026-08-08。
...
- 扫码：生成、待扫码、已扫码待确认、过期、连续检查失败和手动刷新分别建模；同一登录 Destination 内切换登录方式时保留会话并继续轮询，刷新或真正离开登录 Destination 时取消轮询。
...
2026-08-08 腾讯失败返回复验：`21e` 已完成密码页上下文保持、系统居中 Confirm Dialog、白色 Surface、Bold 标题、Tonal 取消与 Filled 重试对照；公共 Dialog 在 Android 窗口显式固定 32% 遮罩，避免设备主题默认值漂移。证据见 [`login-tencent-failure-dialog-2026-08-08.md`](../design/evidence/login-tencent-failure-dialog-2026-08-08.md)。至此登录全部确认稿视觉状态均已关闭；剩余项只包含文末列出的真实服务与用户主动验收，不再阻塞后续页面视觉开发。
