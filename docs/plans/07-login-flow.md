...
当前已完成第 1、2 项及第 3 项的实现：密码请求、风险挑战/方式/提交协议、Repository 领域基础、密码表单、一次重试 ViewModel、短信验证 UI，以及基于 AndroidX WebKit 的非导出腾讯 Activity。普通、凭据错误、风险入口、短信/腾讯验证恢复与 `1.5×` 字体已有截图基准；密码入口、系统返回和腾讯 Activity 非导出门禁已通过 ELE-AL00 / API 29 真机测试且未提交真实凭据。用户主动风控兼容验收尚未完成；固定 Node 请求层生成的模拟 `sid/edt` 不迁移，原生响应缺失和腾讯实际资源域名按审计保留为真机验收项。
