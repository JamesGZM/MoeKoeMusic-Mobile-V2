...
当前已完成第 1、2、3 项及第 4 项的代码/截图子切片：密码请求、风险挑战/方式/提交协议、Repository 领域基础、密码表单、一次重试 ViewModel、短信验证 UI、基于 AndroidX WebKit 的非导出腾讯 Activity、二维码 key/check Endpoint、`0/1/2/4` 类型化 Decoder、扫码成功后的原子会话提交、ZXing 高对比二维码、2 秒生命周期轮询、离页/刷新取消、连续三次失败恢复，以及生成/等待/已扫/过期/失败五态截图。密码入口、系统返回和腾讯 Activity 非导出门禁已通过 ELE-AL00 / API 29 真机测试且未提交真实凭据。第 4 项仅剩该真机上的扫码入口、离页取消和真实二维码可扫/服务兼容验收；用户主动风控兼容验收也尚未完成。固定 Node 请求层生成的模拟 `sid/edt` 不迁移，原生响应缺失和腾讯实际资源域名按审计保留为真机验收项。
