...
| Standard TopBar | `component.navigation` | Stable | `10-user-profile`、`15-toolbar-navigation`、设置、歌单详情和本地音乐 | 页面中心锚定 `18sp / 26sp` 标题；调用方只替换返回/关闭等可见导航图标，尾部图标通过 `MoeStandardTopBarAction` 固定 `20dp` 图形与 `48dp` 触控区，公共组件持有颜色和 Insets |
