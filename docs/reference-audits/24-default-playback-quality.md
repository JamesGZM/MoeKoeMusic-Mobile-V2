...
当前 `AppSettings`/Repository 只含主题、自动跳过、动态取色、歌词附加文本和字号，`DefaultQuality` 仍是不可提交行；见 `core/model/src/main/kotlin/cn/james/music/core/model/settings/AppSettingsModels.kt:18-56`、`data/src/main/kotlin/cn/james/music/data/settings/PreferencesAppSettingsRepository.kt:30-103`、`feature/settings/src/main/kotlin/cn/james/music/feature/settings/SettingsUiModels.kt:180-191`。当前 `KugouPlaybackSourceResolver` 初始化会话后始终调用未带质量的 client 方法；见 `data/src/main/kotlin/cn/james/music/data/kugou/KugouPlaybackSourceResolver.kt:22-65`，而 client 又固定调用默认 `songUrl(songHash)`；见 `kugou-api/src/main/kotlin/cn/james/music/kugou/api/endpoint/KugouOnlineClient.kt:79-103`。
...
2. **迁移事实 + Android 适配决定**：PC 按所选档位建立向低档候选链；Android 登录态先调用一次 `privilege_lite`，复用现有 decoder 对 `level=0`、空 hash、未知质量的过滤和同质量去重，按“用户偏好到 128”的降序选择。候选缺失时以原歌曲 hash 构造同一降序 `song_url` 尝试；这是把 PC 回退规则收束到 Android 已有类型化协议边界，而非新增档位。
3. **迁移事实 + Android 适配决定**：PC 会跳过 `mp4` 或没有 URL 的候选再尝试下一档。Android 当前 `KugouPlaybackAddressDecoder` 先把 HTTP URL 升级为 HTTPS、再过滤非安全 URL；若原始 URL 为空或过滤后为空，只会映射为 `Unavailable(NoCopyright|VipRequired)`，不能区分其原始原因，见 `kugou-api/src/main/kotlin/cn/james/music/kugou/api/endpoint/KugouPlaybackDecoders.kt:94-145`。因此后续 resolver 只可把成功结果中的 `extension=mp4`，及 decoder 已归类的 `VipRequired`，视为当前候选不可用并继续更低档；不得声称 client 能识别“空 URL”或“非 HTTPS URL”的原始原因。第一个安全 URI 成功即停止，结果同时携带实际选中的质量。
4. **Android 新决定**：`NoCopyright` 立即终止，不以低档重试掩盖版权反馈；`VerificationRequired` 与 `AuthenticationRequired` 立即终止，不降级以绕过风控或已失效登录；候选请求或 `song_url` 的离线、超时、连接、服务和协议问题保持原类型并立即终止，不把临时网络问题误判为“换低音质即可”。这些是 Android 的类型化失败恢复边界，不声称为 PC 原样行为。
5. **Android 适配决定**：`CancellationException` 原样抛出；每次地址解析以现有 item/source-refresh 生命周期代际隔离，旧候选、旧 URL 或旧质量不得替换新歌曲、已重试地址或已切换的会话。既有 Media3 HTTP/IO 一次地址刷新仍只重新解析当前项，且会读取刷新开始时的最新成功偏好；不增加第二套刷新或无限候选重试。
