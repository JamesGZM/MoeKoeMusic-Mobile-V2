...
本审计允许实现酷狗在线歌曲的歌词候选搜索、KRC 下载与解包、逐字/翻译/音译解析、类型化领域映射和本地成功缓存。它不授权提前实现歌词 Compose 页面：虽然 [`07-player-lyrics.png`](../design/mockups/07-player-lyrics.png) 已确认主内容态，但加载、无歌词、离线无缓存、协议失败与重试状态仍缺独立静态设计图；必须先用 `frontend-design` 在该基座上补图并由用户确认。
...
已确认 [`07-player-lyrics.png`](../design/mockups/07-player-lyrics.png) 只覆盖正常逐字内容态；[`player-flow`](../design/prototypes/player-flow/README.md) 只验证封面/歌词分页、点击定位、手动滚动暂停跟随与返回层级。它们不能覆盖以下静态状态：
...
固定顺序为：先完成本审计和协议/数据层，再用 `frontend-design` 以 `07` 原页面为基座分别生成单状态图，用户确认后同步 Mockup 索引，最后才实现 `HorizontalPager` 与歌词 Compose。生成覆盖层时仍使用原页面确定性合成，不重新生成底层播放器；单图独立输出，不能把所有状态挤进一张低分辨率总览。
...
4. 设计：以 `07` 基座补齐缺失状态并等待用户确认；确认前不做歌词 Compose。
...
- `:feature:player`（设计确认后）：未请求、加载、内容、无歌词、离线、错误、重试、歌曲切换旧结果隔离；标准/`1.5×`/`2.0×` 截图；逐字高亮不导致整页或封面每帧重组。
...
协议、技术栈、依赖、数据流、缓存、失败恢复与测试矩阵无关键待定项，允许开始 `:kugou-api` 与 `:data` 的歌词协议/数据切片。用户可见歌词 UI 的状态设计尚不完整，因此 Compose 门禁仍关闭；必须先补图并确认，不能用数据实现完成替代视觉门禁。
