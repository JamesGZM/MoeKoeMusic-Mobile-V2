...
5. UI：app-side `AppPlayerLyricsViewModel`、`HorizontalPager`、歌词列表/逐字高亮/点击 seek 和截图；已完成歌词列表自动跟随与手动纵向拖动后 `3500ms` 暂停，恢复时取最新当前行；不改变请求、缓存或 timing 归属。
...
- `:feature:player`：未请求、加载、内容、无歌词、离线、错误、重试、歌曲切换旧结果隔离；标准/`1.5×`/`2.0×` 截图；逐字高亮不导致整页或封面每帧重组；自动跟随覆盖有效行、`3500ms` 边界、连续拖动、旧动画/文档失效及附加文本/字号重新测量。
- 真机手动：进入封面不请求、首次滑到歌词加载、再次进入缓存命中、切歌取消旧结果、点击行 seek、手动滚动暂停跟随、离线缓存/无缓存、三键导航安全区、返回保留播放；本轮尚未在真机验证触控、惯性或横纵手势。
