...
3. **Settings 真实闭环**：036 已完成编码前契约，037 已接入 `CacheMaintenanceRepository` 的确认、独立 clear job、单飞、锁定、类型化成功/partial/failed 反馈与精确整次 Retry。`ClearCache` 是不展示容量或历史值、保留 Chevron 的 Action；以同一 Settings 页面与约 32% 黑色遮罩呈现 `MoeAlertDialog` 危险操作。正文只列首页、歌词、图片和重新下载后果，并明确排除本地音乐、账号、播放记录。提交前 Back/遮罩可关闭，提交中两个按钮禁用、主按钮加载且不允许关闭；成功关闭 Dialog 并显示无 action、约 4 秒后自动消费的 Snackbar，Partial/Failed 关闭 Dialog 并显示带 Retry 的错误 Snackbar，取消不显示普通失败。任一其他 Settings 写入开始时清除旧缓存反馈；若 cache clear 后完成，则它的 typed 反馈重新成为当前可 Retry 项，避免 Snackbar 与 retry request 错配。清理与 CacheLimit 不共用动态区域：前者有独立局部 region，后者继续 Deferred / Unavailable。037 的 JVM/Compose 编译覆盖重复点击单飞、partial/retry、dismiss、其他设置写入不互相取消以及确认/禁用/错误语义；仅新增确认 Dialog 的 `1.0×` / `2.0×` reference，截图 validate、contract、fidelity、人工复核与治理门禁均已通过。
4. **视觉与门禁**：确认/保存/部分失败设计 contract、截图和局部 fidelity 已完成；未改 CacheLimit 行，未放宽阈值或遮罩。受影响 Settings 的 unit/compile/lint、contracts/fidelity/impact/golden、architecture/skill/agent governance 与 diff check 已通过。
...
037 已完成 Settings Compose、JVM/AndroidTest 编译覆盖与两个确认 Dialog reference；截图 validate、fidelity、人工并排/overlay 及最终治理门禁均已通过。本审计与契约未运行模拟器、ADB、真机或真实服务；Coil 默认 disk 目录在目标设备上的实际大小、系统低空间回收、图片 request 与 clear 同时发生的厂家文件系统表现，均留给实现后的指定真机验收；不存在需要用户重新决定的产品或依赖阻塞。
