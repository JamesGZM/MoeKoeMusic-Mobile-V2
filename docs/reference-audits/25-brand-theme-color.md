...
4. Settings：真实选择行、现有 Dialog、独立 generation、回滚与精确 Retry、contract、JVM 与 Compose 语义已于 032 完成；默认天空蓝的 9 张受影响基线与五种非蓝状态、主题色 Dialog（`1.0×` / `2.0×`）的 7 张新基线均已在人工/符合度复核后受限更新，仍待最终 validation 与治理门禁。
