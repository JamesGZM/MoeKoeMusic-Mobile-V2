...
公共组件采用显式影响面登记：歌曲母组件、媒体徽标、搜索输入、TopBar、分隔线和被动描边的源码路径分别绑定其页面 contract。CI 从真实变更文件集合计算受影响页面，要求每个消费者都存在当轮 `passed=true` 的设计符合度结果；未登记或缺失证据直接失败。该门禁解决“母组件改完只复核一个页面、其他消费者到最后才暴露回归”的流程缺口，不依赖自由文本提醒。
