...
- 同一 ViewModel 同时只允许一个提交操作。当前登录页会话内保留各方式的输入草稿；短信倒计时和二维码轮询在同一登录 Destination 内切换方式时继续，返回对应方式时复用原会话。退出登录 Destination 后由 ViewModel 生命周期整体释放敏感状态与任务。
- 二维码轮询间隔 2 秒；手动刷新时取消旧任务并生成新 key，返回离开登录 Destination 或 ViewModel 清除时立即取消。页面会话的本地有效期固定为 120 秒，倒计时减到零时即转为 `Expired` 并停止轮询，不能依赖服务端最终返回过期，也不能停在 `00:00` 继续请求。连续三次网络/协议检查失败进入错误，用户手动刷新生成新 key。
...
- `:feature:login`：三种模式、提交锁定、倒计时、切换保留会话、退出释放任务、一次风控重试、多账号显式选择、二维码连续失败与刷新 ViewModel 测试。
