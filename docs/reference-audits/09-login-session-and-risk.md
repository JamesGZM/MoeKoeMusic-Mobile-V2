...
- `:kugou-api` 已基于 Ktor Client + OkHttp Engine 实现短信和密码登录、临时 AES/RSA 包装、类型化会话、多账号、风险方式与验证提交，以及二维码 key/check Endpoint 和 `0/1/2/4` 类型化状态；`:feature:login` 已实现 ZXing 渲染和生命周期轮询，真机可扫/服务兼容仍待验收，视觉直接按新版单状态确认稿返工。
