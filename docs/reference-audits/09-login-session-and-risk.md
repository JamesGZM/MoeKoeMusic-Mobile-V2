...
- 二维码轮询间隔 2 秒；离开扫码方式、刷新、返回或 ViewModel 清除时立即取消。页面会话的本地有效期固定为 120 秒，倒计时减到零时即转为 `Expired` 并停止轮询，不能依赖服务端最终返回过期，也不能停在 `00:00` 继续请求。连续三次网络/协议检查失败进入错误，用户手动刷新生成新 key。
