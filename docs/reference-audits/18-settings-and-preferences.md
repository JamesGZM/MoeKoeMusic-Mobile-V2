...
8. 后续缓存及其余播放能力分别在真实消费者完成时增加对应设置 Item。

每个切片独立提交、推送并恢复干净工作区。

## 测试与验收

- JVM：默认值、四种主题映射、自动跳过/动态取色/翻译与音译布尔值持久化、读取 IOException、写入失败、取消透传、快速连续写入代际、ViewModel 不乐观覆盖和精确重试；开启/关闭、单曲、连续六次失败与下一次错误生效的播放失败策略。
