...
- 首批同时提供只读的“关于 MoeKoe Air”应用信息。确认稿中的主题色、封面动态取色、播放与音质、歌词、缓存和语言分组，必须等各自真实消费者和失败语义完成后按纵向切片接入。
- 明确非目标：不创建无响应 Item，不为尚未消费的值写入 DataStore，不清理媒体或首页缓存，不重启进程，不新增网络、权限、后台任务、分析或遥测。
...
- 决策：只渲染已工作的主题与关于分组，后续按所属能力纵向增加。确认稿决定分组、Item、间距和滚动视觉，不把示例值变成实现真值。
- 理由：空开关和无效值违反产品完成语义；禁用占位会让页面充满不可操作控件；分阶段渲染保持每个切片可运行。
- 验收：页面不存在无回调 Item、假缓存容量或未消费开关；每个新增设置同提交包含消费者与测试。
...
- 基准画布为 `1024 × 2300` 设计坐标、简体中文、浅色、`1.0×` 字体。运行时使用当前 Composable 内容宽度 `Wc`，统一比例 `s = min(Wc / 1024, 1)` 映射水平边距、分组宽度与视觉间距；触控区仍满足最小 `48dp`，文字使用主题 `sp`，不按原图 px 硬编码。
...
1. 领域端口、`:data` Preferences DataStore、错误与 Repository JVM 测试。
2. app-level 主题消费，替换 `MainActivity` 临时状态，验证重建恢复。
...
- 集成：Activity 重建与进程重启恢复；Settings → Back 返回 My；匿名与登录态均可进入；退出确认仍可达。
