...
- “我的”匿名态的账号资产入口统一复用一个登录目的地；点击签到、VIP、我喜欢、最近播放、云盘、收藏、关注、歌单或用户主页时，从 `MyGraph` 进入现有 `LoginDestination`，不得为每个资产创建重复目的地。登录成功后弹回触发时保留的“我的”栈并刷新真实数据；取消或返回仍回到原匿名页面。本地音乐与应用级设置不触发该门禁。
...
- 匿名态每个账号资产入口都进入同一个登录目的地，且进入登录前不触发受认证资产请求；本地音乐与应用级设置保持直接可达。
- 登录完成回到原“我的”栈并刷新账号数据，取消登录回到原匿名状态且不堆叠重复目的地。
- 重复点击当前 Tab 不堆叠重复目的地。
- Activity 重建后恢复当前目的地。
- Release 导航图不存在播放实验台。
