...
“我的 → 本地音乐”使用 Toolbar 右侧唯一的无文字导入图标进入独立“导入本地音乐”页面；本地库继续提供排序、搜索和删除入口。独立导入页状态固定为：

- 未授权时显示全页权限解释；授权成功后自动开始设备扫描，不要求再次点击扫描按钮。
- 扫描通过 `Flow<DeviceAudioCandidate>` 逐条追加真实 MediaStore 候选；扫描态只读，不显示 Checkbox、全选或导入操作。
- 扫描完成后切换到多选态，才显示 Checkbox、全选与“导入已选”。扫描与选择状态互斥。
- 权限拒绝停留在权限态；扫描失败进入错误态，不允许对不完整候选执行导入。
...
本地库、扫描只读列表和扫描完成多选列表共用 `MoeSongRow`；差异只通过封面、元数据、播放态和尾部操作参数表达。确认设计和状态矩阵见 [`design/evidence/local-music-2026-08-09.md`](design/evidence/local-music-2026-08-09.md)。
