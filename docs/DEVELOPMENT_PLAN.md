...
`:core:designsystem` 已完成 Primary、Typography、Spacing、Shapes、Dimensions、TopBar 和 `MoeSnackbar` 第一批校准；按钮、输入、音乐内容、MoeToast 和页面状态组件仍需按 `DESIGN_SYSTEM.md` 与确认设计稿继续实现。无版权、VIP、网络、会话与协议错误已接入根层类型化 Snackbar，播放地址失效最多刷新一次；真实 CDN 过期与稀有服务错误样本仍需在后续兼容性验收中补证。
