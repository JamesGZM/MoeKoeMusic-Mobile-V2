...
状态：已确认，等待工程初始化。更新日期：2026-08-04。

本文是 MoeKoeMusic Mobile V2 的总开发路线。当前仓库仍处于文档与决策阶段；完成本文档评审前，不创建 Android 工程或迁移业务代码。
...
| 阶段 | 目标 | 主要交付物 | 退出条件 | 状态 |
| --- | --- | --- | --- | --- |
| 0 | 文档与决策 | 本计划、测试策略、本地音乐规格、UI 映射、ADR | 文档互相一致且无关键待定项 | 进行中 |
| 1 | 工程基础 | Gradle 多模块、依赖注入、Design System、CI | Debug App 可安装，基础检查通过 | 未开始 |
| 2 | 播放内核 | MediaLibraryService、队列、通知栏、系统控制 | App 管理的测试音频可稳定后台播放 | 未开始 |
| 3 | 本地音乐 | 选择、扫描、外部导入、去重、列表、删除、播放 | “打开方式 = 导入 + 播放”端到端通过 | 未开始 |
...
本地音乐列表、设备扫描、多选导入、外部打开进度、批量结果及异常状态尚需在阶段 3 开发前补齐视觉规格，但必须沿用上述 Material 3 设计语言，不建立第二套风格。
