...
短信/多账号、密码、安全验证与扫码五态已经进入 `:feature:login` Compose 功能实现；运行时图片由 Coil 处理，二维码由 ZXing Core 3.5.4 生成，正式图标来自 Material Icons。风险确认、短信验证和腾讯失败已从旧稿的卡片替换/独立白页结构改为确认稿要求的居中 Dialog，并以保持不变的密码页作为底层；密码卡已补回确认稿标题并移除与顶部模式选择重复的错误态切换按钮。多账号已覆盖默认不选、显式选择和验证码失效三态，失效后保留选择上下文并提供重新验证手机号的恢复操作。现有登录 Compose 仍需继续收敛旧稿遗留的密度、滚动及其余状态精度。密码入口、系统返回、扫码入口、真实 key、离页切换和二维码解码等既有真机结果仅继续作为行为证据。另一台已登录酷狗设备的状态 `2→4` 实际扫码认证仍需用户主动验收，未提交真实凭据或二维码。
