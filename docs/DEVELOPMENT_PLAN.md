...
首页真实内容与缓存门禁已经通过，见 [`reference-audits/15-home-content-and-cache.md`](reference-audits/15-home-content-and-cache.md)。首批只接入轮播、每日推荐和推荐歌单，按协议与 Decoder、Room v5 完整快照、cache-first Repository、ViewModel、Compose 页面拆成原子切片；排行榜与新歌仍归发现页。缓存按匿名/用户身份分区，15 分钟内不重复自动刷新，部分或失败响应不得覆盖最后完整快照。
