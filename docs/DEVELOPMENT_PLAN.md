...
短信/多账号、密码、安全验证与扫码五态已经进入 `:feature:login` Compose 功能实现；运行时图片由 Coil 处理，二维码由 ZXing Core 3.5.4 生成，正式图标来自 Material Icons。风险确认、短信验证和腾讯失败已从旧稿的卡片替换/独立白页结构改为确认稿要求的居中 Dialog，并以保持不变的密码页作为底层；密码卡已补回确认稿标题并移除与顶部模式选择重复的错误态切换按钮。多账号已覆盖默认不选、显式选择和验证码失效三态，失效后保留选择上下文并提供重新验证手机号的恢复操作。扫码生成、等待、已扫、过期和失败五态已共享确认稿要求的标题、使用说明、三步指引与安全页脚。登录 Compose 已完成本轮确认稿密度、覆盖层、滚动可达性和状态精度收敛，26 张截图基准以及登录方式、多账号和生成进度语义的 API 29 真机自动测试均通过。密码入口、系统返回、扫码入口、真实 key、离页切换和二维码解码等既有真机结果继续作为行为证据。另一台已登录酷狗设备的状态 `2→4` 实际扫码认证仍需用户主动验收，未提交真实凭据或二维码。

“我的”首个账户纵向切片已经贯通 `:kugou-api`、`:data` 与 `:feature:my`：按 `04-my-v4.png` 展示真实用户资料和 VIP 摘要，覆盖匿名、加载、部分失败、刷新与退出确认，不用假资产计数填补尚未迁移的接口；匿名、已认证和 `1.5×` 字体截图基准及 API 29 真机导航验证已经通过。密码与安全验证已完成协议、领域、一次重试状态机、隔离腾讯验证容器，以及新版确认稿要求的风险覆盖层；二维码已完成协议、领域、2 秒生命周期轮询、120 秒本地过期截止、连续三次失败恢复、五态功能 UI 与截图，并通过 API 29 真机入口、真实 key、离页和截图解码门禁。登录视觉已按确认的新版单状态图完成本轮统一返工，腾讯 Activity 非导出门禁也已通过；真实扫码状态 `2→4`、会话恢复和真实风控域名/票据兼容仍需用户主动验收。签到、VIP 领取和音乐库资产属于后续切片。
