...
- ADB、UIAutomator、真机截图、IME、Insets、触控和厂商系统栏验收必须调用 `$moekoe-device-qa`。所有命令绑定用户指定 serial，证据只写入 `build/reports/device-qa/` 并脱敏；普通页面几何仍由 Preview、UI contract 与设计符合度门禁验证。
...
核心页面必须先通过设计符合度，再建立或更新实现回归截图：

- 设计符合度以已确认设计图为期望值。运行截图按页面实际画布边界逆向归一化到原始设计坐标，比较固定锚点、边界、比例、圆角、裁切和累计漂移，并输出半透明叠加或差异图。
- 实现回归以已经通过设计符合度的 Compose 截图为期望值，只防止后续意外变化。重新录制截图后测试通过，不能证明实现符合设计。
- 真实系统时间、状态栏/导航栏图标、动态账号、二维码、验证码和倒计时可按规格遮罩；固定页面结构不得遮罩。
- 更新核心页面截图基准前必须记录设计文件、语言、主题、字体倍率、物理截图、密度、逻辑窗口、当前约束、全部 Insets、归一化比例和锚点误差。
- 登录页面的具体误差和矩阵遵循 [`design/LOGIN_LAYOUT_SPEC.md`](design/LOGIN_LAYOUT_SPEC.md)。

显示矩阵与平台矩阵独立：平台矩阵覆盖 API、权限、生命周期和系统行为；显示矩阵覆盖窗口宽高、密度、Insets、导航模式、主题和字体倍率。一个 API 29 真机结果不能替代不同窗口尺寸，一个截图尺寸也不能替代其他 API 行为。

页面滚动按设计行为测试，不能只断言“最终能看到按钮”：

- 规格为禁止滚动的标准视口，断言纵向滚动轴最大值为 `0`，滑动手势后滚动位置和固定锚点不变。
- 规格为条件滚动的短高度、IME 或大字体视口，只有实际内容溢出时最大值才大于 `0`，首尾关键内容均可达。
- 条件解除后断言位置归零；重叠、负 offset、系统栏 Padding 和不可见 Spacer 不得制造虚假滚动距离。
- 固有滚动列表继续验证稳定 key、位置恢复和边界反馈，不与整页兜底滚动嵌套争抢手势。
...
2. Build Logic 测试、架构边界、UI contract 和 skill 治理门禁。
3. 受影响模块单元测试。
4. 全部 Debug 单元测试。
5. Lint。
