...
- 手机端普通确认型默认宽度为 `min(264.dp, windowWidth - 64.dp)`；六位验证码等结构化输入型允许使用 `min(288.dp, windowWidth - 64.dp)`。这两个规格由 390dp 登录画布校准，位于旧候选小弹窗与 `18-mobile-states-overlays.png` 大示例之间，不再把示例的约 `320.dp` 宽度机械套用到登录流程。大字体下允许内容增高，不固定高度；其他内容不能按文案长度任意放大。
