...
- Dialog 与 Bottom Sheet 状态图必须以对应的原页面设计图为底图，按“原页面 → 统一遮罩 → 独立覆盖层”的顺序确定性合成；禁止使用生图模型重新生成或近似复刻遮罩后的页面。
- 遮罩只能改变底图的整体明度与可交互语义。底层文字、图标、输入值、按钮标签、组件尺寸和坐标必须与原页面逐像素保持一致，不得因覆盖层出现错位、变形或文案漂移。
- 覆盖层自身可以独立设计或生成，但必须先去除其临时背景，再作为前景层放回完整手机画布；每个状态保持一张独立原分辨率设计图，不用横向拼图承载多个状态。
- 评审覆盖层时同时核对底图一致性、遮罩透明度、前景层位置与组件 Token；任一项不一致都不能通过设计确认门禁。
...
- 手机端普通确认型默认宽度为 `min(304.dp, windowWidth - 64.dp)`；六位验证码等结构化输入型允许使用 `min(320.dp, windowWidth - 64.dp)`。390dp 登录画布上的单侧边距分别约为 `43.dp` 与 `35.dp`；该规格介于旧候选偏小尺寸与 `18-mobile-states-overlays.png` 实测约 `350.dp` 的大示例之间。大字体下允许内容增高，不固定高度；扩大容器不得带动字号、验证码格或操作文案同比放大。
...
- 取消操作在左，使用低强调 `primaryContainer` Tonal 背景与 `onPrimaryContainer` 文案；主要操作在右，使用 Filled 主色背景。取消不得只保留裸文字，也不得与主操作使用同等级 Filled 强调。危险操作只将右侧最终确认按钮切换为 `error` 语义色。
...
登录风控等临时阻断确认同样复用此结构：保留遮罩后的原页面和表单草稿，Dialog 相对完整可用视口居中；取消使用低强调 Tonal Button，开始验证使用 Filled Button，两者同高并对齐。不得改造成页面内提示卡或下沉面板。
