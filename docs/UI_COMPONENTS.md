...
| 短时后台任务正在处理 | `MoeSnackbar` 加载态 | 保持上下文，可在完成后替换为结果 |
...
- 使用系统遮罩表达主要模态层级，并在 Sheet 顶缘使用统一、低透明度的轻量上投影辅助分离；不得完全取消顶缘层级，也不得使用四周厚重悬浮阴影。处理手势导航和底部安全区，不得画成居中的桌面悬浮卡片。
- 内容默认使用 `24.dp` 水平内边距；拖拽手柄、标题、内容组、说明组与操作区分别按 `12 / 16 / 20 / 24.dp` Token 建立层级，不得把列表、提示和按钮无间隔堆叠。
- 播放队列支持展开状态；排序、筛选、音质和上下文操作优先使用半高或内容自适应状态。
- 顶部拖拽手柄只表达可拖动性，不替代标题。操作行最小高度 `48.dp`，当前选项同时使用图标或选中标记，不能只依赖颜色。
- Sheet 的主操作默认使用一个全宽 `48.dp` Filled Button；页面级 `56.dp` 大按钮不得机械套用。取消默认使用 `48.dp` TextButton；只有语义上并列且确实需要持续强调的次操作才使用 Outlined Button，禁止堆叠两个同等级全宽大按钮。
...
本地音乐复制属于持续任务：主要进度显示在导入页面；页面可见时 Snackbar 只作辅助。导入成功、部分成功和失败使用 Snackbar 或结果页，不能用 Toast 连续播报。通过 `ACTION_VIEW` 导入时，成功后立即播放；失败反馈不得提供“临时播放原文件”的旁路。
...
登录风控等临时阻断确认同样复用此结构：保留遮罩后的原页面和表单草稿，Dialog 相对完整可用视口居中；不得改造成页面内提示卡、下沉面板或两个等宽大按钮。
