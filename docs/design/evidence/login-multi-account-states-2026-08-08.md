...
- 三态复用登录 Hero、Card 边界和页面设计坐标；标题使用 `50` 设计单位 Medium，标题后的正常间距与内容 top padding 同步校准，说明、三行账号列表、安全提示与操作区保持固定顺序。
...
- `login.multi-account.selected` 保留 Card 顶边探针；严格契约同时覆盖标题、账号行与操作区。验收以统一宽度、局部控件规格和局部间距为硬约束，不以自然可滚动内容的整页底部位置作为锚点。
