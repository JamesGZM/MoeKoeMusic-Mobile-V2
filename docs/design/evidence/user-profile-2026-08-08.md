...
- 比较范围：确认稿移除顶部 `52px` 系统安全区后，裁取至应用级 MiniPlayer 上边界并按宽度归一化，与实现截图顶部 `1024 × 1945px` 对照；MiniPlayer 与系统安全区由 App 壳负责，不重复画入 Feature 截图。
...
- 页面复用已经确认的全局居中 Toolbar：标题使用 `18sp / 26sp`，分享与更多为 `20dp` 可见图标并保留独立 `48dp` 触控区；Hero 紧接 Toolbar，不叠加额外顶部留白。
