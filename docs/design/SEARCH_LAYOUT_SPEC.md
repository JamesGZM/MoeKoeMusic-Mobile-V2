...
- 歌手 Hero 水平边距 `14dp`，圆角 `16dp`，使用 `surfaceContainerLow` 与低对比 `outlineVariant`；头像约 `64dp` 圆形裁切。关注入口保留至少 `56 × 48dp` 语义区域，内部 Filled 可见面约 `56 × 28dp`，不能套用默认高按钮导致 Hero 右侧过重。
- 歌手类型徽标使用 Tertiary 描边语义，与 Primary 关注按钮区分；昵称、统计和代表作保持两级文本，按钮缩窄后不得再无故截断代表作。
- 歌曲区标题左对齐，右侧“查看全部 + Chevron”至少 `48dp`；歌曲行封面约 `52dp`，标题/副标题两行，尾部品质/MV 徽标和更多操作保持紧凑。
- 当前播放歌曲使用 Primary 指示，不改变整行高度。封面失败仍保留相同槽位。
- 歌单与专辑使用横向 `LazyRow`，Compact 卡片宽约 `96dp`，方形封面、圆角 `12dp`、两行文字和可选 `24dp` 播放覆盖；不得把横向卡片改成纵向大卡列表。
...
5. 既有截图基线不能替代上述比较；尤其要复核 Hero 大面积 Surface、Filled 操作尺寸、徽标色调和横卡播放覆盖，不得只看区块是否存在。
