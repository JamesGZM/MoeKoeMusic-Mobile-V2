...
- 自动结果负责来源哈希、尺寸、裁切、动态区域、遮罩和已登记锚点；人工复核负责字体栅格、阴影衰减、图标路径、摄影裁切和整体视觉节奏。
- 全页像素指标可能稀释关键标题、操作按钮和局部排版偏差；已登记 `region.*` 时，局部指标必须与全页指标同时通过，失败时按区域修实现，不得删除区域、扩大区域阈值或转为 mask。
- 结构设计图不承担全部功能状态真值；启禁、加载、错误等必须由状态规则、行为测试和 Design System 组件截图证明。
