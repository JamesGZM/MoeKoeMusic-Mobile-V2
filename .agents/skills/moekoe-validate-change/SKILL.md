...
2. 父智能体检查 `git status --short` 和子智能体报告的差异，区分本切片与用户已有变更；父智能体不得修改任何仓库文件，也不得运行格式化、生成或其他实现性变更操作。验收未通过或验收前不得暂存或提交；验收通过后仅父智能体可以精确暂存并形成原子提交。
...
- 子智能体完成实现、写入、格式化、生成和修复后只报告 diff 与验证证据，绝不执行 `git add`、`git commit`、`git push` 或分支/历史变更。父智能体运行验收命令；其 `build/`、cache 和报告输出允许存在。
- 每个完成且验收通过的可审查切片由父智能体独占形成原子提交；提交前再次核对子智能体交付的 diff、暂存范围与验证证据。
