...
- 登录主状态 `13`、Toolbar、颜色、字体、间距和通用组件图板继续作为已确认基线；旧密码、扫码、安全验证与多账号横向总览稿已撤销实现基线资格，新的独立单状态候选图仍待用户确认。登录协议、状态机、腾讯隔离容器、ZXing 渲染和生命周期轮询已经落地，但现有 Compose 登录布局需在新版候选确认后按真实设计图返工；历史原型和既有真机结果只证明对应交互或协议行为，不证明新版视觉合规。
