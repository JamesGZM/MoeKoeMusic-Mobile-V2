...
- 登录主状态、密码、扫码、安全验证、多账号、Toolbar、颜色、字体、间距和通用组件图板已经确认，Compose Token 与 TopBar 第一批校准已经完成；手机号/多账号、密码、短信安全验证 UI、腾讯隔离验证容器，以及二维码协议、ZXing 高对比渲染和生命周期轮询均已按确认设计图与安全审计落地，交互原型只作状态参考；扫码真机可扫与真实风控兼容验收仍待后续门禁。
