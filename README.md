...
当前开发顺序固定为“品牌与启动基建 → 首页真实闭环 → 我的与登录用户链路 → 播放器完整链路 → 发现页”。首页与发现目前仍是临时页面；全屏播放器只完成封面首个切片且仍有真机交互待验，不能据此跳过前置页面继续扩展。正式产品名与 Android 启动品牌资源归入当前最优先基建切片。
...
- 登录主状态 `13`、新版密码/扫码/安全验证/多账号独立单状态图、Toolbar、颜色、字体、间距和通用组件图板均为已确认基线；旧 `19` 至 `22` 横向总览稿仅保留设计演进记录。登录协议、状态机、腾讯隔离容器、ZXing 渲染和生命周期轮询已经落地，现有 Compose 登录布局需直接按已确认单状态图返工；历史原型和既有真机结果只证明对应交互或协议行为，不证明新版视觉合规。
