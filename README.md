...
- 第一版 UI 视觉语言与核心页面设计稿已经确认；`09` 与 `14` 至 `18` 正在按 `10-user-profile` 的标准 Toolbar 和最新按钮/Dialog 决策修订，候选稿确认前不驱动 Compose 改造。
- 登录主状态 `13`、新版密码/扫码/安全验证/多账号独立单状态图，以及 `10-user-profile` 的标准 Toolbar 样式均为已确认基线；旧 `19` 至 `22` 横向总览稿仅保留设计演进记录，`14` 至 `18` 通用图板修订候选待确认。登录协议、状态机、腾讯隔离容器、ZXing 渲染和生命周期轮询已经落地，现有 Compose 登录布局需直接按已确认单状态图返工；历史原型和既有真机结果只证明对应交互或协议行为，不证明新版视觉合规。
- 音乐内容组件已基于原 `17-music-content-components.png` 接入搜索、本地音乐、MiniPlayer 和队列；浅色、深色、`1.5×` 字体截图及 ELE-AL00 / API 29 真机回归已通过。`17` 修订候选只校准视觉规范，不撤销这些既有行为证据。
